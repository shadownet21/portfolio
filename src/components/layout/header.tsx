...
export function Header({ locale }: { locale: Locale }) {
  ...
  // Keep the visitor on the same page (e.g. a case study) when switching language.
  const pathname = usePathname() ?? `/${locale}`;
  const otherLocaleHref = pathname.replace(new RegExp(`^/${locale}(?=/|$)`), `/${otherLocale}`);
      ...
      <header className="surface fixed inset-x-0 top-0 z-50 border-x-0 border-t-0 bg-[color:var(--surface)]/90 backdrop-blur-xl">
        ...
        <div className="container-shell flex h-20 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <SafeLink href={PLACEHOLDERS.cv} className="button-primary !h-10 !min-h-10 hidden sm:inline-flex" download label={copy.cv}><Download size={17} aria-hidden="true" />CV</SafeLink>
            <button ref={menuButtonRef} type="button" className="button-secondary !h-10 !min-h-10 !w-10 !p-0 lg:hidden" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-menu" aria-label={open ? copy.close : copy.menu}>
